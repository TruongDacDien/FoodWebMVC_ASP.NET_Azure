	...
	public IViewComponentResult Invoke(string revenueOrder)
	{
		...
		switch (revenueOrder)
		{
			...
			case "thisYear":
				...
				objLastRevenue = _context.Orders.Where(x => x.DayOrder.Year == lastYearDate.Year)
					.Sum(x => x.TotalMoney);
				ViewBag.RevenueDay = "Năm này";
				break;
		}

		ViewBag.RevenuePercent =
			objLastRevenue > 0 ? Math.Round((objRevenue - objLastRevenue) * 100, 2) : objRevenue * 100;
		ViewBag.RevenueColor = objRevenue == objLastRevenue ? "text-muted" :
			objRevenue >= objLastRevenue ? "text-success" : "text-danger";
