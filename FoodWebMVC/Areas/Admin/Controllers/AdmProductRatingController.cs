...
public class AdmProductRatingController : Controller
{
	...
	public async Task<IActionResult> Create(
		[Bind("ProductRatingId,Stars,RatingContent,PRDateCreated,ProductId,CustomerId")] ProductRating productRating)
	{
		...
		ViewData["CustomerId"] =
			new SelectList(_context.Customers, "CustomerId", "CustomerPassword", productRating.CustomerId);
	...
	public async Task<IActionResult> Edit(int id,
		[Bind("ProductRatingId,Stars,RatingContent,PRDateCreated,ProductId,CustomerId")] ProductRating productRating)
	{
		if (id != productRating.ProductRatingId) return NotFound();
		...
		if (ModelState.IsValid)
		{
			...
			{
				if (!ProductRatingExists(productRating.ProductRatingId)) return NotFound();
	...
	public async Task<IActionResult> DeleteConfirmed(int id)
	{
		...
		var productRating = await _context.ProductRatings.FindAsync(id);
		if (productRating != null) _context.ProductRatings.Remove(productRating);
