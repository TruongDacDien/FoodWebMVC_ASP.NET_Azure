...
public class AdmStatisticsController : Controller
{
	...
	public AdmStatisticsController(IProductRepository productRepository, IOrderDetailRepository orderDetailRepository,
	...
	{
		var amount = data.Count(); // số lượng cột của chart
	...
	public IActionResult StaticProductAmount(int option = 0)
	{
		...
		var productAmount = new Bar(amount); //Setup các thông số cho chart
		SetupBarChart(productAmount, data);
		productAmount.options.plugins.title.text =
			"Số lượng sản phẩm bán được trong " + TimeOption(option); //Đặt tên cho chart
	...
	public IActionResult StaticProductRevenue(int option = 0)
	{
		...
		//Lấy data từ cơ sở dữ liệu
		var data = _productRepository.GetProductWithRevenue(time.startDate, time.endDate)
			.OrderByDescending(x => x.Value).ToArray();
		var amount = data.Count(); // số lượng cột của chart

		//Tạo chart mới với số cột amount
		var productRevenue = new Bar(amount);
		SetupBarChart(productRevenue, data); //Setup các thông số cho chart
		productRevenue.options.plugins.title.text =
			"Doanh thu sản phẩm bán được trong " + TimeOption(option); // Đặt tên cho chart
	...
	{
		var text = option == 0 ? "Doanh số" : "Doanh thu";
		ViewData["option"] = option;
	...
	public IActionResult RevenueStructure(int? option)
	{
		...
		var amount = data.Count();
		var revenueStructure = new Pie(amount);
